...
class DockerAllContainers extends StatefulWidget {
  @override
  _DockerAllContainersState createState() => _DockerAllContainersState();
}

class _DockerAllContainersState extends State<DockerAllContainers> {
  bool loading = false;
  ...
  terminateContainer(imageid) async {
    myToast('Please Wait...', Colors.blue);
    setState(() {
      loading = true;
    });
    var url = Uri.parse(
        'http://${ip}/cgi-bin/docker/docker/terminatecontainer.py?x=$imageid');
    try {
      var response = await http.get(url, headers: {
        "Accept": "application/json",
        "Access-Control_Allow_Origin": "*"
  ...
  getAllContainer() async {
    ...
    var url = Uri.parse('http://${ip}/cgi-bin/docker/docker/allcontainers.py');
    try {
      var response = await http.get(url, headers: {
        "Accept": "application/json",
        "Access-Control_Allow_Origin": "*"
  ...
  slidelist(index) {
    ...
    var startStopColor = Colors.red.shade300;
    var startStopText = "STOP";
    var startStopIcon = Icons.stop;

    if (finalSplitString.contains("Exited")) {
      statusColor = Colors.red;
      startStopColor = Colors.green;
      startStopText = "START";
      startStopIcon = Icons.play_circle_filled_sharp;
    } else {
      statusColor = Colors.green;
      startStopText = "STOP";

      startStopColor = Colors.red.shade300;
    }
